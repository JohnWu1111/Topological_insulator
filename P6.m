...
% H = 3*( kron(sigma_y,H2) + kron(sigma_z,H3)) + kron(eye(2),H4);
...
for t = 1:L
    for i = 1:Lx
        for j = 1:Ly
            a = (i-1)*Ly+j;
            b = L+(i-1)*Ly+j;
            phi_xy1(i,j) = phi((i-1)*Ly+j,t);
            phi_xy2(i,j) = phi(L+(i-1)*Ly+j,t);
        end
    end
...
Jy1_left = sum(Jy1(1,:))
Jy1_right = sum(Jy1(end,:))
Jy2_left = sum(Jy2(1,:))
Jy2_right = sum(Jy2(end,:))
Jy_left = sum(Jy(1,:))
Jy_right = sum(Jy(end,:))

figure;
quiver(30*Jx1',zeros(Ly,Lx),0)
hold on
quiver(zeros(Ly,Lx),30*Jy1',0)

figure;
quiver(30*Jx2',zeros(Ly,Lx),0)
hold on
quiver(zeros(Ly,Lx),30*Jy2',0)
